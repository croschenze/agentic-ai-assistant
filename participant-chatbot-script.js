...
class ParticipantInterface {
    constructor() {
        ...
        this.translations = {
            en: {
                ...
                aiGreeting: 'Hello! I am your chatbot. I can help you browse web pages, analyze content, answer questions, etc. What can I help you with?',
            ...
            zh: {
                ...
                aiGreeting: '您好！我是您的聊天机器人。我可以帮助您浏览网页、分析内容、回答问题等。请告诉我您需要什么帮助？',
    ...
    addMessage(sender, text, avatarName = null, avatarIcon = null) {
        ...
        if (sender === 'ai' || sender === 'wizard') {
            ...
            senderName.textContent = avatarName || 'Chatbot';
    ...
    updateControlPanelLanguage() {
        ...
        const creativityLabel = creativitySlider ? creativitySlider.closest('.setting-group').querySelector('.setting-label') : null;
        if (creativityLabel) {
            creativityLabel.textContent = this.t('creativity');
        }
        
        const personalitySlider = document.querySelector('#personality-slider');
        const personalityLabel = personalitySlider ? personalitySlider.closest('.setting-group').querySelector('.setting-label') : null;
        if (personalityLabel) {
            personalityLabel.textContent = this.t('personality');
        }
        
        const workstyleSlider = document.querySelector('#workstyle-slider');
        const workstyleLabel = workstyleSlider ? workstyleSlider.closest('.setting-group').querySelector('.setting-label') : null;
        if (workstyleLabel) {
            workstyleLabel.textContent = this.t('workingStyle');
        }
        
        // 更新滑块左右标签
        const creativityContainer = creativitySlider ? creativitySlider.closest('.slider-container') : null;
        const creativityLabels = creativityContainer ? creativityContainer.querySelectorAll('.slider-labels span') : [];
        if (creativityLabels.length >= 2) {
            creativityLabels[0].textContent = this.t('rigor');
            creativityLabels[1].textContent = this.t('creative');
        }
        
        const personalityContainer = personalitySlider ? personalitySlider.closest('.slider-container') : null;
        const personalityLabels = personalityContainer ? personalityContainer.querySelectorAll('.slider-labels span') : [];
        if (personalityLabels.length >= 2) {
            personalityLabels[0].textContent = this.t('calm');
            personalityLabels[1].textContent = this.t('energetic');
        }
        
        const workstyleContainer = workstyleSlider ? workstyleSlider.closest('.slider-container') : null;
        const workstyleLabels = workstyleContainer ? workstyleContainer.querySelectorAll('.slider-labels span') : [];
...
function setupModeToggle() {
    ...
    // 如果控制面板元素不存在，直接返回
    if (!modeToggleBtn || !modeTitle || !modeIcon) {
        return;
    }
