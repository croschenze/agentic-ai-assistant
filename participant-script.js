...
class ParticipantInterface {
    ...
    updateControlPanelLanguage() {
        ...
        if (widthLabel) {
            widthLabel.textContent = this.t('width');
        }
        
        const heightLabel = document.getElementById('height-label');
        if (heightLabel) {
            heightLabel.textContent = this.t('height');
        }
        
        const samplingStepsLabel = document.getElementById('sampling-steps-label');
        if (samplingStepsLabel) {
            samplingStepsLabel.textContent = this.t('samplingSteps');
        }
        
        const batchCountLabel = document.getElementById('batch-count-label');
        if (batchCountLabel) {
            batchCountLabel.textContent = this.t('batchCount');
        }
        
        const batchSizeLabel = document.getElementById('batch-size-label');
        if (batchSizeLabel) {
            batchSizeLabel.textContent = this.t('batchSize');
        }
        
        // 更新小标题
        const sectionTitles = document.querySelectorAll('.section-title');
        if (sectionTitles.length >= 2) {
            sectionTitles[0].textContent = this.t('imageSize');
            sectionTitles[1].textContent = this.t('batch');
        }
    }
...
function initializeControlPanel() {
    ...
    safeSetupModeToggle();
    safeSetupSliders();
    safeSetupImageSettings();
}

// 安全设置模式切换
function safeSetupModeToggle() {
    try {
...
// 安全设置滑块
function safeSetupSliders() {
    try {
        setupSliders();
    } catch (error) {
        console.error('设置滑块时出错:', error);
    }
}

// 安全设置图片编辑功能
function safeSetupImageSettings() {
    try {
        // 延迟设置，确保DOM完全加载
        setTimeout(() => {
            const sliders = [
            ...
            sliders.forEach(({ slider, input }) => {
                setupSliderInputSync(slider, input);
            });
        }, 100);
    } catch (error) {
        console.error('设置图片编辑功能时出错:', error);
    }
}
...
function setupSliderInputSync(sliderId, inputId) {
    ...
    if (slider && input) {
        ...
        console.warn(`滑块同步设置失败: ${sliderId} 或 ${inputId} 元素未找到`);
    }
}
